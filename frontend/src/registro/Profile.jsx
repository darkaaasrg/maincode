...
    
    // Стан завантаження ТІЛЬКИ для фото
...
            });
            ...
            // Обробка не-JSON відповідей (наприклад, 429 Too Many Requests)
            const contentType = response.headers.get("content-type");
            if (!contentType || !contentType.includes("application/json")) {
                 const textError = await response.text();
                 // Встановлюємо помилку, але не "ламаємо" сторінку
                 setError(textError || 'Помилка сервера (429 - Too Many Requests)');
                 // Не кидаємо throw, щоб finally спрацював
            } else {
...
    };
    
    // 3. ОБИРАЄМО І МИТТЄВО ЗАВАНТАЖУЄМО ФОТО
    const handlePhotoChangeAndUpload = async (e) => {
        const file = e.target.files[0];
        if (!file) {
            return; // Користувач закрив вікно, нічого не обравши
        }

        setIsUploading(true);
        setMessage('Завантаження фото...'); // Повідомляємо про початок
        setError('');
...
            const data = await response.json();
            if (response.ok) {
                setMessage(data.message); // "Фото профілю успішно оновлено!"
